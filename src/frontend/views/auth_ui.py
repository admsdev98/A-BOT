...
from services.utils import return_mail_message_template


@st.dialog("A-BOT te tiene en la mira 🎯")
def show_auth_dialog():
    ...
    if st.session_state.get("auth_failure_reason") == "attempts_exceeded":
        ...
        st.markdown("Necesitas iniciar sesión para poder chatear con A-BOT. Esto nos ayuda a garantizar un uso responsable y que todos puedan interactuar con A-BOT")
    ...
    if st.session_state.get("auth_failure_reason"): 
        ...
        st.markdown(f'''
        <a class="auth-btn linkedin-btn" href="{formatted_mail}" target="_blank" rel="noopener noreferrer">
        ...
        st.markdown("Necesitas iniciar sesión. Esto nos ayuda a garantizar un uso responsable y que todos puedan interactuar con A-BOT")
        ...
        st.warning(
            "🔒 **Solo validamos tu identidad**, no almacenamos tus datos personales para otros fines. "
            "Puedes obtener más información visitando nuestra [Política de Cookies](?section=cookies)."
        )
